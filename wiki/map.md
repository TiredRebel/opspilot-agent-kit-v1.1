...
| RAG service | services/rag/app/{main,llm,retrieval,schemas,db,settings}.py | built (P1-1..P1-5, P4-1 stats ext, P5 multi-provider) | 18 L1/L2 tests, all pass, against a dedicated `<POSTGRES_DB>_test` DB (gotcha #31) | `LLM_PROVIDER` selects among `anthropic` (primary, falls back to `openai` on 5xx/timeout/connection — the only pair with a fallback chain), `openai`, `gemini` (gemini-2.5-flash + gemini-embedding-001, plain REST via httpx, rate-limit retry — gotchas #34/#35), `ollama` (local, OpenAI-compatible endpoint, untested live in this environment — gotcha #36), and `fake`; all cost-logged; all 6 endpoints live |
