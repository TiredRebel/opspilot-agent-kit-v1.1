...
| RAG service | services/rag/app/{main,llm,retrieval,schemas,db,settings}.py | built (P1-1..P1-5, P4-1 stats ext, P5/P6 multi-provider) | 18 L1/L2 tests, all pass, against a dedicated `<POSTGRES_DB>_test` DB (gotcha #31) | `LLM_PROVIDER` selects among `anthropic` (primary, falls back to `openai` on 5xx/timeout/connection — the only pair with a fallback chain), `openai`, `gemini` (gemini-2.5-flash + gemini-embedding-001, plain REST via httpx, rate-limit retry — gotchas #34/#35), `ollama` (local, OpenAI-compatible endpoint, live-verified for `/classify` — best result 0.833 with a 12B model, gotchas #38/#40), and `fake`; `_parse_json()` tolerates markdown-fenced structured output across all providers (gotcha #39); all cost-logged; all 6 endpoints live |
...
| Evals | evals/{tickets.jsonl,conftest,test_classify,test_grounding}.py | built (P5-1/P5-2) | L3 | **0.85 accuracy not yet reached live** — best result 0.833 (12B local Ollama model), see PROGRESS.md Blockers + gotchas #33–#40 for the full provider/model comparison; `test_grounding.py` not yet passing (Gemini quota + no 1536-dim local embed model) |
| CI | .github/workflows/ci.yml | built (P5-3) | — | lint+L1/L2 on push/PR (no secrets), evals on `workflow_dispatch`; not yet exercised against a real GitHub Actions run |
| Deploy | docs/infrastructure.md, docker-compose.yml (`prod` profile), caddy/Caddyfile, scripts/{backup.sh,test_backup_restore.sh} | built (P6-1/P6-2), **not executed against a real VM** (deferred — dev setup stays the working baseline) | M7 (open) | Caddy behind `prod` profile, two subdomains (api./ops.), no path-based routing; backup/restore cycle verified live against a scratch DB, `rclone` upload step documented but not exercised |
...
ADR-001 LLM-via-service · ADR-002 confidence gate (blend + 0.70 threshold) · ADR-003 pgvector over external vector DB · ADR-004 asyncpg/no-ORM · ADR-005 single Telegram entry point (WF-1 only; WF-3 has no trigger). All five written as of P6-3.
