...
Defines the append-only `ticket_events` audit log (`db/init/02_ticket_events.sql`, ADR-006):
trigger-based capture of ticket lifecycle events from all writers (n8n postgres nodes and the
rag-api alike), database-enforced append-only semantics, the additive-only schema-freeze
amendment, and the `GET /tickets/{ticket_id}/events` read path. `tickets.status` remains the
source of truth — this is an audit log, not event sourcing; intent-level events are deferred to
a future change where the service owns ticket writes.
